...
function ModeSidebar({ dashMode, onSwitchMode, title, subtitle, children }) {
  return (
    <aside className="hidden lg:flex flex-col w-[232px] fixed left-0 top-0 h-screen text-white z-40" style={{ background: "linear-gradient(180deg,#0A3E8F 0%,#071E4A 42%,#003B8F 100%)" }}>
      ...
      <div className="px-3 py-3 grid grid-cols-3 gap-1 border-b border-white/10">
        {MODES.map(([m, l]) => (
          <button key={m} onClick={() => onSwitchMode(m)}
            className={`px-2 py-1.5 rounded-lg text-[11px] font-bold transition cursor-pointer ${dashMode === m ? "bg-white text-brand-navy" : "text-white/65 bg-white/5 hover:bg-white/10"}`}>{l}</button>
        ))}
      </div>
...
function SidebarFlatNav({ items, active, onSelect, accent = "#071E4A" }) {
  return (
    <>
      {items.map(t => {
        ...
        return (
          <button key={t.id} onClick={() => onSelect(t.id)}
            className={`w-full text-left px-3 py-2 rounded-lg text-[13px] font-semibold transition cursor-pointer flex items-center gap-2 ${on ? "bg-white text-brand-navy" : "text-white/65 hover:bg-white/[0.08] hover:text-white"}`}>
