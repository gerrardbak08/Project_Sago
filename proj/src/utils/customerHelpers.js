...
function cFilter(D, yr) {
  if (!yr || yr === "all") return {
    ...
    causes3: (D.causes3 || []).map(c => ({...c, _show:c.n})),
    typeCause:   D.typeCause   ? { cols: D.typeCause.cols,   rows: D.typeCause.all }   : null,
    causeAction: D.causeAction ? { cols: D.causeAction.cols, rows: D.causeAction.all } : null,
  ...
  return {
    ...
    causes3: (D.causes3 || []).map(c => ({...c, _show:c[yk] || 0})).filter(c => c._show > 0).sort((a,b) => b._show - a._show),
    typeCause:   D.typeCause   ? { cols: D.typeCause.cols,   rows: D.typeCause[yk] }   : null,
    causeAction: D.causeAction ? { cols: D.causeAction.cols, rows: D.causeAction[yk] } : null,
