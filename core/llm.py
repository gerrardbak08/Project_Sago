...
출력 스키마 (Tool Use로 강제됨):
  - 위험_요약: str
  - 주요_위험유형: str
  - 오늘의_특별_주의사항: list[{수칙, 근거_사례, 관련_피처}]  (없으면 빈 배열)
  - 특별주의사항_없음_이유: str  (오늘의_특별_주의사항이 빈 배열일 때)
  - 상시_주의사항: list[{수칙, 근거_사례}]  (없으면 빈 배열)
  - 상시주의사항_없음_이유: str  (상시_주의사항이 빈 배열일 때)
...
1. **오늘의 조건과 연관된 사례**: 오늘의 기상 **또는 매장 환경 피처**와 명확히 인과관계가 있는 사례

   **기상 조건 연관 예시:**
   - 오늘 precipitation_sum=15mm → 과거의 '빗물에 미끄러져 넘어짐' 사례
   - 오늘 temperature_2m_min=-3°C → 과거의 '입구 결빙으로 낙상' 사례
   - 오늘 wind_speed_10m_max=12m/s → 과거의 '강풍으로 간판 전도' 사례

   **매장 환경 조건 연관 예시:**
   - 오늘 매장인원=5명(소규모) → 과거의 '혼자 무거운 박스 운반 중 허리 부상' 사례 (인원 부족으로 2인 작업 불가)
   - 오늘 일평균물동량=500박스(고물동) → 과거의 '입고 작업 중 지게차·적재물 충돌' 사례
   - 오늘 창고=30평(대형 창고) → 과거의 '창고 렉 최상단 낙하물 사고' 사례
   - 오늘 평수=50평(소형 매장) → 과거의 '좁은 통로에서 고객·직원 충돌' 사례
   - 오늘 일평균매출=1억(고매출·고혼잡) → 과거의 '혼잡한 매장에서 고객 넘어짐' 사례
   - 오늘 형태=유통점 → 과거의 '유통점 특성상 관리 사각지대 사고' 사례
...
## 매장 환경 피처 활용 지침
기상 조건이 평이하더라도 매장 환경 피처를 반드시 분석하여 오늘의 특별 주의사항을 도출하십시오:
...
- `today_alert_cases`는 **반드시 3건 이상 5건 이하**를 선정하십시오. 과거 사례가 부족하더라도 가장 관련성 높은 사례를 선정해야 합니다.
- `related_feature`에는 "precipitation_sum=15mm" 또는 "매장인원=5명, 일평균물동량=500박스"처럼 피처명=값 형태로 구체적 수치를 명시하십시오.
- `today_special_precautions`이 빈 배열인 경우, `no_special_precautions_reason`에 이유를 반드시 작성하십시오.
- `regular_precautions`이 빈 배열인 경우, `no_regular_precautions_reason`에 이유를 반드시 작성하십시오.
- `today_alert_cases`의 `incident_id`는 입력으로 제공된 사례의 ID(예: cust_0123)를 그대로 사용하십시오.
- 기상 조건이 평이하더라도 매장 환경 피처를 분석하여 반드시 오늘의 특별 주의사항을 1개 이상 도출하십시오.
"""
            ...
            "json": {
                ...
                "properties": {
                    ...
                    "today_special_precautions": {
                        "type": "array",
                        "description": "오늘의 기상·매장 조건과 명확히 연관된 안전 수칙",
                        "items": {
                            "type": "object",
                            "properties": {
                                "precaution": {
                                    "type": "string",
                                    "description": "안전 수칙 본문",
                                },
                                "evidence_case": {
                                    "type": "string",
                                    "description": "이 수칙의 근거가 된 과거 사례 요약",
                                },
                                "related_feature": {
                                    "type": "string",
                                    "description": "관련 피처와 값. 예: precipitation_sum=15mm",
                                },
                            },
                            "required": ["precaution", "evidence_case", "related_feature"],
                        },
                    },
                    "regular_precautions": {
                        "type": "array",
                    ...
                    "today_alert_cases": {
                        ...
                        "items": {
                            ...
                            "required": ["incident_id", "incident_content", "selection_reason"],
                        },
                    },
                    "additional_notes": {
                        "type": "string",
                    ...
                    "no_special_precautions_reason": {
                        "type": "string",
                        "description": "오늘의 특별 주의사항이 없는 경우 그 이유. 예: '오늘은 기상 조건이 평이하여 특별한 조건 연관 위험이 없습니다.'",
                    },
                    "no_regular_precautions_reason": {
                        "type": "string",
                        "description": "상시 주의사항이 없는 경우 그 이유. 예: '과거 사례 중 조건 무관 부주의 사례가 충분하지 않습니다.'",
                    },
                },
                "required": [
                    "risk_summary",
