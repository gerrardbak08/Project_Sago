...
- `today_alert_cases`는 3건 이상 5건 이하여야 합니다.
- `related_feature`에는 "precipitation_sum=15mm"처럼 피처명=값 형태로 구체적 수치를 명시하십시오.
- `today_special_precautions`은 오늘의 기상·매장 조건과 명확히 연관된 사례에서 도출한 수칙만 포함하십시오.
- `regular_precautions`은 조건 무관 상시 부주의 사례에서 도출한 수칙만 포함하십시오.
...
def _translate_response(raw: dict) -> dict:
    ...
    for en_key, ko_key in _KEY_MAP_TOP.items():
        if en_key not in raw:
            continue
        value = raw[en_key]

        if en_key == "today_special_precautions" and isinstance(value, list):
            result[ko_key] = [
                {_KEY_MAP_SPECIAL.get(k, k): v for k, v in item.items()}
                for item in value
                if isinstance(item, dict)
            ]
        elif en_key == "regular_precautions" and isinstance(value, list):
            result[ko_key] = [
                {_KEY_MAP_REGULAR.get(k, k): v for k, v in item.items()}
                for item in value
                if isinstance(item, dict)
            ]
        elif en_key == "today_alert_cases" and isinstance(value, list):
            result[ko_key] = [
