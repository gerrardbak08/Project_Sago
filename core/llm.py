...
from core.safety_rules import get_rules_for_prompt
...
4. **모든 수칙 작성 기준 (today_precautions · negligence_precautions 공통)**

   수칙은 반드시 다음 4요소를 포함하십시오:
   - **타이밍**: "지금", "출근 즉시", "오전 입고 전", "매 2시간마다", "영업 시작 전" 등 언제 해야 하는지 명시
   - **장소**: "입구 자동문 안쪽 3m", "창고 1번 통로", "냉장 코너 앞 바닥" 등 실제 매장 내 위치
   - **트리거**: "빗물 자국이 있으면", "박스가 통로를 막고 있으면" 등 행동을 일으키는 조건
   - **구체적 행동**: "드라이걸레로 닦은 뒤", "표지판을 세우세요", "내려놓고 동료를 부르세요" 등 즉시 취할 행동

   **today_precautions의 첫 번째 항목은 반드시 "지금 당장 5분 안에 실행 가능한" 수칙으로 작성하십시오.**

   **좋은 수칙 vs 나쁜 수칙 예시**

   ❌ "미끄러짐에 주의하세요."
   ✅ "지금 입구 자동문 안쪽 3m 바닥을 확인하세요. 빗물 자국이나 우산물이 고여 있으면 드라이걸레로 즉시 닦고 '미끄러움 주의' 표지판을 입구 양쪽에 세우세요."

   ❌ "적재물 정리를 잘 하세요."
   ✅ "오전 입고 시작 전 지금 창고 통로 폭이 1m 이상인지 확인하세요. 전날 입고된 박스가 통로를 막고 있으면 벽면 쪽으로 재적재하고 통로를 확보해야 입고 작업 중 충돌 사고를 막을 수 있습니다."

   ❌ "무거운 물건을 조심해서 드세요."
   ✅ "10kg 이상 박스를 혼자 들 때는 반드시 무릎을 굽히고 허리가 아닌 다리 힘으로 드세요. 들어 올리다 허리에 뻐근함이 느껴지면 즉시 내려놓고 동료를 불러 2인 1조로 작업하세요."

   ❌ "고객 클레임 응대 시 주의하세요."
   ✅ "고객이 흥분하여 목소리를 높이면, 자리를 이동하여 분리된 공간에서 대화하세요. 혼자 대응하기 어려운 상황이면 매니저에게 즉시 무선으로 연락하고 절대 혼자 해결하려 하지 마세요."

   - **사고 발생 건수나 통계 수치는 절대 언급하지 마십시오** (예: "N건 발생" 금지)
   - `negligence_precautions`의 각 문장도 동일한 4요소 기준으로 작성하십시오.
...
- **간략·명료**: 수칙 1개 = 1~2문장. 핵심 행동 하나만 담는다. 긴 설명은 두 개로 나눈다.
"""
...
def build_user_prompt(
    ...
    # B — 데이터 기반 정적 수칙 (dominant type 추출)
    _sorted_types = sorted(type_dist, key=lambda k: type_dist[k], reverse=True)
    _source = leaf_data.get("source", "cust")
    _base_rules = get_rules_for_prompt(
        dominant_types=_sorted_types[:2],
        source=_source,
        max_rules=5,
    )
...
## 이 유형의 핵심 안전수칙 (기반 원칙 — 간략·명료하게 적용)
아래는 이 사고유형에서 실증된 핵심 수칙입니다.
오늘의 기상·매장 조건에 맞게 구체화하되, 수칙은 1~2문장·즉시 실행 가능하게 작성하십시오.
